...
router.get('/', requireLogin, (req, res) => {
    ...
    try {
        stats.totalApplicants = db.prepare(`
            SELECT COUNT(*) AS count
            FROM applicants
        `).get().count || 0;

        stats.inProcess = db.prepare(`
            SELECT COUNT(*) AS count
            FROM applicants
            WHERE application_status IN ('applied', 'in_review', 'interview', 'offered')
        `).get().count || 0;

        stats.hired = db.prepare(`
            SELECT COUNT(*) AS count
            FROM applicants
            WHERE application_status = 'hired'
        `).get().count || 0;

        stats.activeEmployees = db.prepare(`
            SELECT COUNT(*) AS count
            FROM employees
        ...
        stats.avgDaysToHire = avgDaysRow && avgDaysRow.avg_days
        ...
        stats.offerAcceptanceRate = offersTotal > 0
            ? ((hiresTotal / offersTotal) * 100).toFixed(1)
            : 0;

        const departmentsCountRow = db.prepare(`
            SELECT COUNT(*) AS count
            FROM departments
        `).get();
