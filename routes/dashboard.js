...
router.get('/', requireLogin, async (req, res) => {
    ...
    try {
      ...
      FROM applicants
    `);
        stats.totalApplicants = Number(totalApplicantsRow?.count || 0);
    ...
    `);
        stats.inProcess = Number(inProcessRow?.count || 0);
    ...
    `);
        stats.hired = Number(hiredRow?.count || 0);
    ...
    `);
        stats.activeEmployees = Number(activeEmployeesRow?.count || 0);

        const avgDaysRow = await db.get(`
      SELECT AVG(EXTRACT(EPOCH FROM (e.hire_date::timestamp - a.applied_at::timestamp)) / 86400.0) AS avg_days
      FROM employees e
      JOIN applicants a ON e.applicant_id = a.applicant_id
      WHERE e.hire_date IS NOT NULL
        AND a.applied_at IS NOT NULL
    `);
        stats.avgDaysToHire = avgDaysRow?.avg_days ? Number(avgDaysRow.avg_days).toFixed(1) : 0;
      ...
      FROM applicants
    `);
        const offersTotal = Number(offersRow?.offers_total || 0);
        const hiresTotal = Number(offersRow?.hires_total || 0);
        stats.offerAcceptanceRate = offersTotal > 0 ? ((hiresTotal / offersTotal) * 100).toFixed(1) : 0;

        const departmentsCountRow = await db.get(`
      SELECT COUNT(*) AS count
      FROM departments
    `);
        stats.departmentsCount = Number(departmentsCountRow?.count || 0);
    ...
    `);
        stats.openJobs = Number(openJobsRow?.count || 0);
